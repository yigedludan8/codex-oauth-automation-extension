...
function createSignupAuthRestartCurrentAttemptError(detailText = '') {
  const detail = String(detailText || '').replace(/\s+/g, ' ').trim();
  const suffix = detail ? `页面提示：${detail}` : '页面提示注册认证阶段发生异常。';
  return new Error(
    `${SIGNUP_AUTH_RESTART_CURRENT_ATTEMPT_ERROR_PREFIX}步骤 3：检测到注册认证错误页（fraud_guard），需要重新开始当前轮。${suffix}`
  );
}
...
function getSignupPasswordFieldErrorText() {
  ...
  if (text && (SIGNUP_PHONE_PASSWORD_MISMATCH_PATTERN.test(text) || SIGNUP_PHONE_ALREADY_EXISTS_PATTERN.test(text))) {
    return text;
  }
  ...
  if (passwordInput) {
    ...
    if (wrapperText && (SIGNUP_PHONE_PASSWORD_MISMATCH_PATTERN.test(wrapperText) || SIGNUP_PHONE_ALREADY_EXISTS_PATTERN.test(wrapperText))) {
      return wrapperText;
    }
  }
...
function getAuthTimeoutErrorPageState(options = {}) {
  ...
  const signupRestartCurrentAttemptBlocked = /\bfraud_guard\b/i.test(text);
  const emailInUseBlocked = /email_in_use/i.test(text);
  const userAlreadyExistsBlocked = /user_already_exists/i.test(text);

  if (!titleMatched && !detailMatched && !routeErrorMatched && !fetchFailedMatched && !maxCheckAttemptsBlocked && !signupRestartCurrentAttemptBlocked && !emailInUseBlocked && !userAlreadyExistsBlocked) {
    return null;
  }
  ...
  return {
    ...
    signupRestartCurrentAttemptBlocked,
    emailInUseBlocked,
...
async function recoverCurrentAuthRetryPage(payload = {}) {
  ...
  while (clickCount < maxClickAttempts) {
    ...
    if (retryState.signupRestartCurrentAttemptBlocked && (flow === 'signup' || flow === 'signup_password')) {
      throw createSignupAuthRestartCurrentAttemptError(retryState.text || getPageTextSnapshot());
    }
  ...
  if (finalRetryState.signupRestartCurrentAttemptBlocked) {
    throw createSignupAuthRestartCurrentAttemptError(finalRetryState.text || getPageTextSnapshot());
  }
...
function inspectSignupVerificationState() {
  ...
  if (isSignupPasswordErrorPage()) {
    ...
    return {
      ...
      signupRestartCurrentAttemptBlocked: Boolean(timeoutPage?.signupRestartCurrentAttemptBlocked),
      retryPageText: String(timeoutPage?.text || ''),
...
async function prepareSignupVerificationFlow(payload = {}, timeout = 30000) {
  ...
  while (Date.now() - start < timeout && recoveryRound < maxRecoveryRounds) {
    ...
    if (snapshot.state === 'error') {
      if (snapshot.signupRestartCurrentAttemptBlocked) {
        throw createSignupAuthRestartCurrentAttemptError(snapshot.retryPageText || getPageTextSnapshot());
      }
...
async function waitForVerificationSubmitOutcome(step, timeout) {
  ...
  while (Date.now() - start < resolvedTimeout) {
    ...
    if (step === 4 && retryState?.signupRestartCurrentAttemptBlocked) {
      throw createSignupAuthRestartCurrentAttemptError(retryState.text || getPageTextSnapshot());
    }
  ...
  if (step === 4) {
    ...
    if (signupRetryState?.signupRestartCurrentAttemptBlocked) {
      throw createSignupAuthRestartCurrentAttemptError(signupRetryState.text || getPageTextSnapshot());
    }
