(function attachBackgroundPhoneVerification(root, factory) {
  ...
  function createPhoneVerificationHelpers(deps = {}) {
    const {
      ...
      DEFAULT_SIGNUP_TEMP_NUMBER_DURATION_HOURS = 12,
    } = deps;
    ...
    async function requestPhoneActivation(state = {}, options = {}) {
      ...
      for (let round = 1; round <= maxAcquireRounds; round += 1) {
        ...
        for (const attempt of countryAttempts) {
          ...
          if (options?.useSignupTempNumber) {
            const tempNumberDurationHours = normalizeSignupTempNumberDurationHours(
              state?.signupPhoneTempNumberDurationHours
            );
            try {
              await addLog(
                `步骤 2：已启用临时取号，正在通过 HeroSMS getRentNumber 获取 ${countryConfig.label} 号码（duration=${tempNumberDurationHours}）。`,
                'info'
              );
              ...
              lastError = error;
              lastFailureText = String(error?.message || error || '').trim();
              noNumbersByCountry.push(`${countryConfig.label}: ${lastFailureText || 'getRentNumber failed'}`);
