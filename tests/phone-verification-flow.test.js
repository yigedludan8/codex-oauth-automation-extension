...
test('signup phone helper uses getRentNumber when temporary signup phone toggle is enabled', async () => {
      ...
      if (action === 'getRentNumber') {
        return {
          ...
          text: async () => JSON.stringify({
            activationId: '353101410',
            phoneNumber: '447436759015',
            countryCode: 16,
            serviceCode: 'dr',
          }),
        };
  ...
  const activation = await helpers.prepareSignupPhoneActivation({
    ...currentState,
    heroSmsCountryId: 16,
    heroSmsCountryLabel: 'United Kingdom',
  });

  assert.equal(activation.activationId, '353101410');
  assert.equal(activation.phoneNumber, '447436759015');
  const rentRequest = requests.find((request) => request.searchParams.get('action') === 'getRentNumber');
  assert.ok(rentRequest);
  assert.equal(rentRequest.searchParams.get('api_key'), 'demo-key');
  assert.equal(rentRequest.searchParams.get('duration'), '12');
  assert.ok(!requests.some((request) => request.searchParams.get('action') === 'getNumber'));
});

test('signup phone helper uses configured getRentNumber duration when temporary signup phone toggle is enabled', async () => {
  const requests = [];
  let currentState = {
    heroSmsApiKey: 'demo-key',
    signupPhoneUseTempNumber: true,
    signupPhoneTempNumberDurationHours: 18,
  };
  const helpers = api.createPhoneVerificationHelpers({
    addLog: async () => {},
    ensureStep8SignupPageReady: async () => {},
    fetchImpl: async (url) => {
      ...
      if (action === 'getRentNumber') {
        return {
          ok: true,
          text: async () => JSON.stringify({
            activationId: '353101411',
            phoneNumber: '447436759016',
            countryCode: 16,
            serviceCode: 'dr',
          }),
        };
  ...
  await helpers.prepareSignupPhoneActivation({
    ...currentState,
    heroSmsCountryId: 16,
    heroSmsCountryLabel: 'United Kingdom',
  });
  ...
  assert.ok(rentRequest);
  assert.equal(rentRequest.searchParams.get('api_key'), 'demo-key');
  assert.equal(rentRequest.searchParams.get('duration'), '18');
});
...
test('signup temporary phone helper releases rented number after repeated SMS timeout', async () => {
  const setStateCalls = [];
  const statusActions = [];
  let currentState = {
    heroSmsApiKey: 'demo-key',
    ...
    signupPhoneNumber: '447436759015',
    signupPhoneVerificationPurpose: 'signup',
    signupPhoneActivation: {
      activationId: 'signup-rent-123',
      phoneNumber: '447436759015',
      provider: 'hero-sms',
      serviceCode: 'dr',
      countryId: 16,
  ...
  const helpers = api.createPhoneVerificationHelpers({
    ...
    getState: async () => currentState,
    sendToContentScriptResilient: async (_source, message) => {
      if (message.type === 'RESEND_SIGNUP_PHONE_VERIFICATION_CODE') {
        return { success: true };
      }
      throw new Error(`Unexpected content-script message: ${message.type}`);
    },
...
test('phone verification helper polls and parses SMSBower verification codes', async () => {
  ...
  const helpers = api.createPhoneVerificationHelpers({
    ...
    fetchImpl: async (url) => {
      const parsedUrl = new URL(url);
      assert.equal(parsedUrl.searchParams.get('action'), 'getStatus');
      assert.equal(parsedUrl.searchParams.get('id'), 'sb-123');
      assert.equal(parsedUrl.searchParams.get('api_key'), 'smsbower-key');
      pollCount += 1;
      return {
        ok: true,
        status: 200,
        text: async () => (pollCount === 1 ? 'STATUS_WAIT_CODE' : 'STATUS_OK:998877'),
      };
    },
    getState: async () => ({
      phoneSmsProvider: 'smsbower',
...
test('phone verification helper rejects SMSBower reuse and ignores temporary signup number flow during signup acquisition', async () => {
  ...
  await assert.rejects(
    () => helpers.reactivatePhoneActivation(
      {
        phoneSmsProvider: 'smsbower',
        smsBowerApiKey: 'smsbower-key',
      },
    ...
    /SMSBower does not support activation reuse/
  );
