...
test('auto-run controller stops after 10 whole-round exit fallback restarts on final round', async () => {
  const events = {
    logs: [],
    broadcasts: [],
    accountRecords: [],
  ...
  const controller = api.createAutoRunController({
    ...
    getNodeIdsForState: () => Object.values(TEST_STEP_NODE_IDS),
    getPendingAutoRunTimerPlan: () => null,
    getRunningNodeIds: () => [],
    getState: async () => ({
      ...currentState,
  ...
  assert.equal(events.runCalls, 11, 'should allow at most 10 whole-round exit fallback restarts before stopping');
  assert.equal(events.accountRecords.length, 1, 'final fallback exhaustion should persist one failed round record');
  assert.equal(events.accountRecords[0].status, 'failed');
  assert.ok(events.logs.some(({ message }) => /已达到 10 次上限/.test(message)));
  assert.equal(events.broadcasts.some(({ phase }) => phase === 'stopped'), true, 'after exhausting fallback restarts the auto-run should stop');
});
