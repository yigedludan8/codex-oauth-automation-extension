...
const PERSISTED_SETTING_DEFAULTS = {
  ...
  signupPhoneTempNumberDurationHours: DEFAULT_SIGNUP_TEMP_NUMBER_DURATION_HOURS,
...
function normalizeSignupTempNumberDurationHours(value, fallback = DEFAULT_SIGNUP_TEMP_NUMBER_DURATION_HOURS) {
  const rawValue = String(value ?? '').trim();
  const numeric = Number(rawValue);
  if (!rawValue || !Number.isFinite(numeric)) {
    return Math.min(
      ...
      Math.max(
        ...
        Math.floor(Number(fallback) || DEFAULT_SIGNUP_TEMP_NUMBER_DURATION_HOURS)
      )
    );
  }
  return Math.min(
    SIGNUP_TEMP_NUMBER_DURATION_HOURS_MAX,
    Math.max(SIGNUP_TEMP_NUMBER_DURATION_HOURS_MIN, Math.floor(numeric))
  );
}
...
function normalizePersistentSettingValue(key, value) {
  switch (key) {
    ...
    case 'signupPhoneUseTempNumber':
      return Boolean(value);
    case 'signupPhoneTempNumberDurationHours':
      return normalizeSignupTempNumberDurationHours(value, DEFAULT_SIGNUP_TEMP_NUMBER_DURATION_HOURS);
...
async function getPersistedAliasState() {
  try {
    ...
    const usedSignupPhoneNumbers = normalizeBooleanMap(stored.usedSignupPhoneNumbers);
    return {
      manualAliasUsage,
      preservedAliases,
      usedSignupPhoneNumbers,
...
async function setState(updates) {
  ...
  if (Object.keys(updates || {}).length > 0) {
    ...
    if (Object.prototype.hasOwnProperty.call(updates, 'usedSignupPhoneNumbers')) {
      persistentAliasUpdates.usedSignupPhoneNumbers = normalizeBooleanMap(updates.usedSignupPhoneNumbers);
    }
...
function isRestartCurrentAttemptError(error) {
  ...
  const message = String(typeof error === 'string' ? error : error?.message || '');
  return /当前邮箱已存在，需要重新开始新一轮|SIGNUP_PHONE_PASSWORD_MISMATCH::|SIGNUP_AUTH_RESTART_CURRENT_ATTEMPT::|SIGNUP_PHONE_ALREADY_EXISTS::/i.test(message);
}
...
async function restartSignupPhonePasswordMismatchAttemptFromStep(step, restartCount, error, options = {}) {
  ...
  const reasonLabel = String(options.reasonLabel || '手机号/密码不匹配').trim();
